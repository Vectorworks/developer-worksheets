...
- [Specialized for Railing/Fence](#Specialized-for-Railing/Fence)
...
Returns the current time as a 12h formatted time string (hh-mm am/pm).


```python
=Time12h returns the current time as a 12 hour formatted time string (hh-mm am/pm).

```

#### __Time24h()__ ####

Returns the current time as a 24h formatted time string (hh-mm).


```python
=Time24h returns the current time as a 24 hour formatted time string (hh-mm).
...
```python
=FDate(1) returns the current date in the first supported date format, the MDYY date format, 1/12/26.
...
14	m/d/yy hms (AM/PM)
15	day, month d, yyyy
16	day, month d, yyyy hms (AM/PM)
17	dow, mmmm d, yyyy
18	dow, mmmm d, yyyy hms (AM/PM)
19	day, d month yyyy
20	day, d month yyyy hms (AM/PM)
21	dow, d mmm yyyy
22	dow, d mmm yyyy hms (AM/PM)
23	d/m/yy hms (AM/PM)
24	day, yyyy month d
25	day, yyyy month d hms (AM/PM)
26	dow, yyyy mmm d
27	dow, yyyy mmm d hms (AM/PM)
28	yy/m/d hms (AM/PM)
29	yyyymmdd
30	mm/dd/yyyy
31	yyyy-mm-dd
32	yyyymmddhhmmss
33	dd/mm/yy
34	dd/mm/yy hms (AM/PM)
35	dd/mm/yyyy
36	dd/mm/yyyy hms (AM/PM)
37	yymmdd
```
...
The following values are supported: "name", "function", "class", "class desc", "class description", "thickness", "lambda", "u-value", "r-value", "net area", "net volume".
...
- <optional param> - Units display
	Whether or not to show the unit mark after the number. Only applicable when the output is in length, area, or volume units and not FeetNInches.
		NoUnits
		Units

- <optional param> - trailing zero display
		When enabled, the resulting string will include trailing zeroes up to the specified decimal precision. Only applicable when the output is in length, area, or volume units and not FeetNInches.
		NoTrailingZero
		TrailingZero
