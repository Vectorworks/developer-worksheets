...
Based on Vectorworks 2027 (Build 882075)
...
- [Specialized for Bridle](#Specialized-for-Bridle)
...
- [Specialized for WinDoor 6.0](#Specialized-for-WinDoor-6.0)
...
Returns the current date. The value returned is a number that represents the day. The cell must be formatted as Date to display the result as a formatted time string.
...
Returns the current time. The value returned is a number that represents the time. The cell must be formatted as Date to display the result as a formatted time string.
...
Returns the current time as a 12h formatted time string (e.g. 11:00 PM).


```python
=Time12h returns the current time as a 12 hour formatted time string (e.g. 11:00 PM).

```

#### __Time24h()__ ####

Returns the current time as a 24h formatted time string (e.g. 23:00).


```python
=Time24h returns the current time as a 24 hour formatted time string (e.g. 23:00).

```

#### __FDate(dateformat)__ ####

Returns the current date as a formatted date string using the specified date format (click on Show Examples to see date format selectors for supported date formats).


```python
=FDate(1) returns the current date in the first date format selector, the MDYY date format, 6/16/25.

Date Formats:

Selector    Display Style                      Format

1           6/16/25                            m/d/yy
2           6/16/25 11:00                      m/d/yy hmm
3           16/6/25                            d/m/yy
4           16/6/25 11:00                      d/m/yy hmm
5           25/6/16                            yy/m/d
6           25/6/16 11:00                      yy/m/d hmm
7           16-Jun-25                          d-mmm-y
8           16-Jun                             d-mmm
9           Jun-25                             mmm-yy
10          11:00                              h mm
11          11:00:02                           h mm ss
12          11:00 AM                           h mm(AM/PM)
13          11:00:02 AM                        h mm ss(AM/PM)
14          6/16/25 11:00:02 AM                m/d/yy hms(AM/PM)
15          Monday, June 16, 2025              day, month d, yyyy
16          Monday, June 16, 2025 11:00:02 AM  day, month d, yyyy hms(AM/PM)
17          Mon, Jun 16, 2025                  dow, mmm d, yyyy
18          Mon, Jun 16, 2026 11:00:02 AM      dow, mmm d, yyyy hms(AM/PM)
19          Monday, 16 June 2025               day, d month yyyy
20          Monday, 16 June 2025 11:00:02 AM   day, d month yyyy hms(AM/PM)
21          Mon, 16 Jun 2025                   dow, d mmm yyyy
22          Mon, 16 Jun 2025 11:00:02 AM       dow, d mmm yyyy hms(AM/PM)
23          16/6/25 11:00:02 AM                d/m/yy hms(AM/PM)
24          Monday, 2025 June 16               day, yyyy month d
25          Monday, 2025 June 16 11:00:02 AM   day, yyyy month d hms(AM/PM)
26          Mon, 2025 Jun 16                   dow, yyyy mmm d
27          Mon, 2025 Jun 16 11:00:02 AM       dow, yyyy mmm d hms(AM/PM)
28          25/6/16 11:00:02 AM                yy/m/d hms(AM/PM)
29          20250616                           yyyymmdd
30          06162025                           mmddyyyy
31          2025-06-16                         yyyy-mm-dd
32          20250616110002                     yyyymmddhhmmss
33          160625                             ddmmyy
34          160625 11:00:02 AM                 ddmmyy hms(AM/PM)
35          16062025                           ddmmyyyy
36          16062025 11:00:02 AM               ddmmyyyy hms(AM/PM)
37          250616                             yymmdd
...
Returns the current date as a formatted date string using the format MDY.


```python
=DateShortMDY returns the current date as a formatted date string using the format MDY, like 11/26/25.

```

#### __DateShortDMY()__ ####

Returns the current date as a formatted date string using the format DMY.


```python
=DateShortDMY returns the current date as a formatted date string using the format DMY, like 26/11/25.

```

#### __DateMedium()__ ####

Returns the current date as a formatted date string using a medium date format.


```python
=DateMedium returns the current date as a formatted date string using a medium date format like: Nov 26, 2025.

```

#### __DateLong()__ ####

Returns the current date as a formatted date string using a long date format.


```python
=DateLong returns the current date as a formatted date string using a long date format like: 
...
Returns the full path of the current document. If the current document is a working file, returns the path of the project file.


```python
=Filepath returns the full path of the current document. If the current document is a working file, returns the path of the project file.

```

#### __Filename()__ ####

Returns the name of the current document. If the current document is a working file, returns the name of the project file.


```python
=Filename returns the name of the current document. If the current document is a working file, returns the name of the project file.

```

#### __ActiveWorksheet()__ ####
...
Returns the DBD BIM Region ID value.


```python
=DBDRegionID 	returns DBD BIM Region ID information.
...
#### __ObjectData('bottom Z bound ground')__ ####

Returns the bottom Z value of the bounding box of the object. The result is relative to the ground.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bottom Z bound ground') returns the bottom Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the ground.

Database Cell:
=ObjectData('bottom Z bound ground') returns the the bottom Z value of the bounding box of the object. The result is relative to the ground.
```

#### __ObjectData('bottom Z bound layer')__ ####

Returns the bottom Z value of the bounding box of the object. The result is relative to the object's layer elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bottom Z bound layer') returns the bottom Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the layer elevation.

Database Cell:
=ObjectData('bottom Z bound layer') returns the the bottom Z value of the bounding box of the object. The result is relative to the layer elevation.
```

#### __ObjectData('bottom Z bound story')__ ####

Returns the bottom Z value of the bounding box of the object. The result is relative to the object's story elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bottom Z bound story') returns the bottom Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the story elevation.

Database Cell:
=ObjectData('bottom Z bound story') returns the the bottom Z value of the bounding box of the object. The result is relative to the story elevation.
```

#### __ObjectData('bounding box dim X')__ ####

Returns the X dimension of the bounding box of the object.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bounding box dim X') returns the X dimension of the bounding box of the symbol whose symbol name is 'Chair-1'.

Database Cell:
=ObjectData('bounding box dim X') returns the X dimension of the bounding box of the object.
```

#### __ObjectData('bounding box dim Y')__ ####

Returns the Y dimension of the bounding box of the object.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bounding box dim Y') returns the Y dimension of the bounding box of the symbol whose symbol name is 'Chair-1'.

Database Cell:
=ObjectData('bounding box dim Y') returns the Y dimension of the bounding box of the object.
```

#### __ObjectData('bounding box dim Z')__ ####

Returns the Z dimension of the bounding box of the object.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'bounding box dim Z') returns the Z dimension of the bounding box of the symbol whose symbol name is 'Chair-1'.

Database Cell:
=ObjectData('bounding box dim Z') returns the Z dimension of the bounding box of the object.
```
...
#### __ObjectData('class tags')__ ####

Returns the tags assigned to the class of the object.


```python
Spreadsheet Cell:
=ObjectData(L='First Floor', 'class tags') returns the tags assigned to the class of the object on the 'First Floor' layer.

Database Cell:
=ObjectData('class tags') returns the tags assigned to the class of the object.
```
...
The following values are supported: "name", "function", "class", "class desc", "class description", "thickness", "lambda", "u-value", "r-value", "net area", "net volume", "height".

"height" is only supported for wall components.
...
Examples:
	...
	# return component height (wall components only)
	ObjectData('component', 'Height')			# core component
	ObjectData('component', 'Height', 2)		# component with index = 2
	

```

#### __ObjectData('dbd bim'; param_name [; <child_param_name>]; index [; <component index>])__ ####

Returns the DBD BIM value for specified parameter name for main object or component. When called on a component, the function will return the specified parameter value for that component.


```python
Examples:
	Database header cell:
	=ObjectData('DBD BIM', 'CAFM-Connect', 1) Returns the value of the "DBD_Layer1.CAFM-Connect" parameter from the DBD BIM Data for each object in the DB row.
	
	=ObjectData('DBD BIM', 'Abdichtungen in / unter Wänden - Kunststoffbahn / Elastomerbahn', 'Baustoff, Abdichtung', 0) Returns the value of the "DBD_Abdichtungen in / unter Wänden - Kunststoffbahn / Elastomerbahn.Baustoff, Abdichtungt" parameter from the DBD BIM Data for each object in the DB row.

	=ObjectData('DBD BIM', 'BIMKey', 0, 1) Returns the value of the "DBD_BIMKey" parameter from the DBD BIM Data for the first component for each object in the DB row.
	
	Spreadsheet cell:
	=ObjectData(SEL=TRUE, 'DBD BIM', 'CAFM-Connect', 1) Returns the value of the "DBD_Layer1.CAFM-Connect" parameter from the DBD BIM Data for the selected object.
	
	=ObjectData(SEL=TRUE, 'DBD BIM', 'Abdichtungen in / unter Wänden - Kunststoffbahn / Elastomerbahn', 'Baustoff, Abdichtung', 0) Returns the value of the "DBD_Abdichtungen in / unter Wänden - Kunststoffbahn / Elastomerbahn.Baustoff, Abdichtungt" parameter from the DBD BIM Data for the selected object.
	
	=ObjectData(SEL=TRUE, 'DBD BIM', 'BIMKey', 0, 1) Returns the value of the "DBD_BIMKey" parameter from the DBD BIM Data for the first component for the selected object.
```
...
#### __ObjectData('insertion point X')__ ####

Returns the X coordinate of the insertion point of the object. The result is relative to the user origin.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'insertion point X') returns the X coordinate of the insertion point of the symbol whose symbol name is 'Chair-1'. The result is relative to the user origin.

Database Cell:
=ObjectData('insertion point X') returns the X coordinate of the insertion point of the object. The result is relative to the user origin.
```

#### __ObjectData('insertion point Y')__ ####

Returns the Y coordinate of the insertion point of the object. The result is relative to the user origin.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'insertion point Y') returns the Y coordinate of the insertion point of the symbol whose symbol name is 'Chair-1'. The result is relative to the user origin.

Database Cell:
=ObjectData('insertion point Y') returns the Y coordinate of the insertion point of the object. The result is relative to the user origin.
```

#### __ObjectData('insertion point Z')__ ####

Returns the Z coordinate of the insertion point of the object. The result is relative to the ground.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'insertion point Z') returns the Z coordinate of the insertion point of the symbol whose symbol name is 'Chair-1'. The result is relative to the ground.

Database Cell:
=ObjectData('insertion point Z') returns the Z coordinate of the insertion point of the object. The result is relative to the ground.
```

#### __ObjectData('insertion point Z layer')__ ####

Returns the Z coordinate of the insertion point of the object. The result is relative to the object's layer elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'insertion point Z layer') returns the Z coordinate of the insertion point of the symbol whose symbol name is 'Chair-1'. The result is relative to the object's layer elevation.

Database Cell:
=ObjectData('insertion point Z layer') returns the Z coordinate of the insertion point of the object. The result is relative to the object's layer elevation.
```

#### __ObjectData('insertion point Z story')__ ####

Returns the Z coordinate of the insertion point of the object. The result is relative to the object's story elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'insertion point Z story') returns the Z coordinate of the insertion point of the symbol whose symbol name is 'Chair-1'. The result is relative to the object's story elevation.

Database Cell:
=ObjectData('insertion point Z story') returns the Z coordinate of the insertion point of the object. The result is relative to the object's story elevation.
```
...
#### __ObjectData('layer tags')__ ####

Returns the tags assigned to the layer of the object.


```python
Spreadsheet Cell:
=ObjectData(L='First Floor', 'layer tags') returns the tags assigned to the layer of the object on the 'First Floor' layer.

Database Cell:
=ObjectData('layer tags') returns the tags assigned to the layer of the object.
```
...
#### __ObjectData('power info'; <Connector Index>; '<DataSelector>'; [<ConnectedTo Index>])__ ####

Access power connector data of an object. Data selectors - 'Name', 'ConnectedToDeviceName', 'ConnectedToComponentName', 'Voltage', 'Connector'.



#### __ObjectData('story level type')__ ####

Returns the level type associated with the story layer of the object.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'story level type') returns the level type associated with the story layer of the symbol whose symbol name is 'Chair-1'.

Database Cell:
=ObjectData('insertion point Z story') returns the level type associated with the story layer of the object.
```

#### __ObjectData('story prefix-suffix')__ ####

Returns the story prefix or suffix associated with the story layer of the object.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'story prefix-suffix') returns the story prefix or suffix associated with the story layer of the symbol whose symbol name is 'Chair-1'.

Database Cell:
=ObjectData('insertion point Z story') returns the story prefix or suffix associated with the story layer of the object.
```

#### __ObjectData('top Z bound ground')__ ####

Returns the top Z value of the bounding box of the object. The result is relative to the ground.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'top Z bound ground') returns the top Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the ground.

Database Cell:
=ObjectData('top Z bound ground') returns the the top Z value of the bounding box of the object. The result is relative to the ground.
```

#### __ObjectData('top Z bound layer')__ ####

Returns the top Z value of the bounding box of the object. The result is relative to the object's layer elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'top Z bound layer') returns the top Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the layer elevation.

Database Cell:
=ObjectData('top Z bound layer') returns the the top Z value of the bounding box of the object. The result is relative to the layer elevation.
```

#### __ObjectData('top Z bound story')__ ####

Returns the top Z value of the bounding box of the object. The result is relative to the object's story elevation.


```python
Spreadsheet Cell:
=ObjectData(S='Chair-1', 'top Z bound story') returns the top Z value of the bounding box of the symbol whose symbol name is 'Chair-1'. The result is relative to the story elevation.

Database Cell:
=ObjectData('top Z bound story') returns the the top Z value of the bounding box of the object. The result is relative to the story elevation.
```

#### __ObjectData('trim height')__ ####

Returns the trim height of the rigging object, as shown in the OIP.
...
Returns the name of the style used by the object.


```python
Database cell:
=StyleName returns the name of the style for the object

Spreadsheet cell:
=StyleName(T='WALL') returns the name of the style for the object whose type is wall

```

#### __IsStyled()__ ####

Returns TRUE if the object is styled, otherwise returns FALSE.
...
Returns the object's fill type.


```python
Database header cell:
=FillType returns the fill type for each object in the database

Spreadsheet cell:
=FillType(sel=true) returns the fill type of the selected objects in the drawing

```

#### __PenType()__ ####

Returns the object's pen type.


```python
Database header cell:
=PenType returns the pen type for each object in the database

Spreadsheet cell:
=PenType(sel=true) returns the pen type of the selected objects in the drawing
...
Returns the value of the specified property for the material resource or object's material.
...
Returns the currently active phase of the drawing.



#### __PhaseCreated()__ ####

Returns the phase in which the object was created.



#### __PhaseRemoved()__ ####

Returns the phase in which the object was removed.



#### __PhaseRelocated()__ ####

Returns the phase in which the object was relocated.



#### __PhaseIsReuse()__ ####

Returns TRUE if the “Reuse” option is selected for the object.



#### __PhaseIsTemporary()__ ####

Returns TRUE if the “Temporary” option is selected for the object.



#### __PhaseUseAltAttribute()__ ####

Returns TRUE if the “Use alternate attributes” option is selected for the demolished or removed object.



#### __PhaseStatus('phase name')__ ####

Returns the status of the object in the specified phase.


```python
Example:
	=IFS ((PHASESTATUS('Phase 1')='New'), MATERIALVOLUME, ELSE, 0)
```

#### __PresentInPhase('phase name')__ ####

Returns TRUE if the object is present in the specified phase.



#### __PhaseIfcStatus('phase name')__ ####

Returns the IFC status in the specified phase of objects that meet the specified criteria.
Objects are classified as NEW (the object is created in this phase),
DEMOLISHED (the object is removed in this phase), or
EXISTING (the object was created earlier and not removed in this phase).
Objects with the “Temporary” option enabled are classified as TEMPORARY.
The first phase is an exception: it contains only EXISTING objects.


```python
Example:
	=IFS((PHASEIFCSTATUS('Phase 1')='EXISTING'), MATERIALVOLUME, ELSE, 0)
```

#### __PhaseIfcExport()__ ####

Returns the phase specified for export in the Export IFC Project dialog. If no phase is explicitly selected, the function defaults to returning the active phase.
...
## Specialized for Bridle


#### __ObjectData('bridle leg'; '<DataSelector>'; '[<PartsSpecifier>]'; [<Leg Index>]__ ####

Get bridle leg data. Data selectors - 'Name', 'Parts Ordered', 'Parts Summed'. Parts specifiers - 'Leg', 'Basket/Pickup', or leave blank to show all parts. If the function is called on a bridle leg subpart there is no need to specify the leg index. If the function is called on a bridle object you have to specify a leg index. You may skip the parts specifier argument entirely when you want to see all parts and specifiy a leg index - e.g. '('bridle leg', 'Parts Ordered', 2)'
...
Access cable break data. Data selectors - 'Name', 'Tape'. If the function is called on a cable break subpart there is no need to specify the break index. If the function is called on a cable object you have to specify the break index.



#### __ObjectData('cable section'; '<DataSelector>'; [<Section Index>)]; [<Part Index>)]__ ####

Access cable section data. Data selectors - 'Name', 'Parts Ordered', 'Break Name', 'Position', 'Box ID', 'Loom ID', 'Tape'. 'Parts Ordered' and 'Position' are read only. If the function is called on a cable section subpart the section index is ignored. If the function is called on a cable object you have to specify the section index. Optionally, for 'Parts Ordered' you can specify part index which to display, or 0 to ignore this argument.



#### __Length('cable section'; '<DataSelector>'; [<Section Index>)]__ ####

Access cable section length. Data selectors - 'Length', 'Start Slack', 'End Slack', 'Swag', 'Total Vertical Drop'. 'Length' and 'Total Vertical Drop' are read only. If the function is called on a cable section subpart there is no need to specify the section index. If the function is called on a cable object you have to specify the section index.
...
Get parameter value or attached record field from the device's corresponding equipment item or rack frame.
...
#### __ObjectData('distributor connections'; '<DataSelector>'; <Port Start>; [<Port End>])__ ####

Access distributor connection data for a range of ports. Data selectors - 'Output', 'FixtureID', 'Dimmer', 'Unit Number', 'Position', 'Universe', 'UniverseAddress'.



#### __Length('device component'; '<component name>')__ ####

Get the circuit length of a distributor object's device component by specifying the name of the device component.
...
#### __ObjectData('group allocation')__ ####

Return the group allocation of the object. For a plant listed in the Plant Information of a Landscape Area, returns the group allocation of the containing Landscape Area. For a Plant object, returns its own group allocation.
...
#### __ObjectData('roof pitch')__ ####

Returns the pitch of the roof or roof face object.


```python
Spreadsheet Cell:
=ObjectData(((L='First Floor') & (T=ROOF)), 'roof pitch') returns the pitch of the roof on the 'First Floor' layer.

Database Cell:
=ObjectData('roof pitch') returns the pitch of the roof or roof face object.
```
...
#### __Length('eval socket device'; 'Device'; 'DistributorObject'; 'Circuit Length')__ ####

Get the circuit length of this socket from its device's Spotlight distributor.
...
#### __ObjectData('area name')__ ####

Returns the name of a Space Area sub part.

The function is applicable on any Detailed Space Area subobject including Net and Gross.
...
This function is applicable to the Space and its Detailed Areas subobjects. When applied to Space Object, it returns the Net Area. 
Optional Area name: this parameter applies to Space object and then will return the area of the given Detailed Area.



#### __Area('space area'; area name)__ ####

This function is applicable to the Space object. It returns the Net Area of the given Detailed Space Area.



#### __Perimeter()__ ####

This function is applicable to the Space and its Detailed Areas subobjects. When applied to Space Object, it returns the Net Perimeter.



#### __Perimeter('gross')__ ####

Returns the perimeter of the Gross Area



#### __Perimeter('holes')__ ####

Returns the sum of the perimeters of all the holes from the Net Area



#### __Perimeter('net')__ ####

Returns the perimeter of the Net Area



#### __Perimeter('space area'; area name)__ ####

This function is applicable to the Space object. It returns the Net Perimeter of the specified Detailed Area.
...
=TXT(<numeric value>, [optional parameters]) - Format the numeric value using the given parameters.
=TXT(<numeric value>, <Unit>, [optional parameters]) - Convert the numeric value from the current document units to the specified units and format it using the given parameters.
=TXT(<numeric value>, <Unit>, <Unit>, [optional parameters]) - Convert the numeric value from the units specified in the first <Unit> parameter to the units speficied in the second <Unit> parameter and format it using the given parameters.

The default states of some of the optional parameters are determined by the output units or the current document settings.
...
Parameter Details:
 - <Unit>
	...
	Weight specific units options (one of the list):
		Milligrams, Grams, Kilograms, MetricTon, Ounces, Pounds, Stone, Quarter, HundredweightUK, HundredweightUS, ShortTonsUS, LongTonsUK

- <optional param> - Number format
	These can not be used together with a <Unit> parameter.
...
- <optional param> - Units display
		Whether or not to show the unit mark after the number. Only applicable when the output is in length, area, volume, or weight units and not FeetNInches.
		NoUnits
		Units

- <optional param> - trailing zero display
		When enabled, the resulting string will include trailing zeroes up to the specified decimal precision. Only applicable when the output is in length, area, volume, or weight units and not FeetNInches.
		NoTrailingZero
		TrailingZero
...
- <optional param> - AlwaysShowSign
		If provided, will always show the sign of the number, even if positive. By default, only negative numbers show the sign.

- <optional param> - Space between value and units, not applicable for FeetNInches output
		NoSpaceAfterValue
		SpaceAfterValue
...
=TXT(2.45, 'Inches', 'Meters') - converts 2.45 inches to meters
=TXT(186, 'SquareInches', 'SquareMeters', 'TrailingZero', '0.000', 'Units', 'NoSpaceAfterValue') - '0.120sq m'
=TXT(1500, 'Grams') - converts 1500 of the document's current mass unit to grams
=TXT(5, 'Kilograms', 'Pounds') - converts 5 kilograms to pounds
